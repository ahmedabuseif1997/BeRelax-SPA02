...
| 0.9 | **Region chosen and the reasoning written down** | Engineering decides, the business signs it off | §11.7(1). **Open, and currently wrong.** The scratch project built while validating the schema (`berelax-crm`) was created in `ap-south-1` (Mumbai) on latency grounds — which is not the criterion this step is about, and contradicts the recommendation in this very row. It is empty and disposable; **the production project must be created in the region chosen here, and Mumbai should not be copied forward by default.** There is no UAE Supabase region; Frankfurt (`eu-central-1`) has the stronger "adequate protection" argument. **Record the choice and why in `docs/compliance/data-processing-register.md`** — that record is the first thing anyone will ask for. Put the API in the same jurisdiction, so the privacy notice names one country and not two. |
