...
| 0.8 | **Supabase / Vercel / Sentry / Cloudflare accounts on paid plans**, owned by the business | The business | **Verified against Supabase's own documentation, not assumed — the Free plan cannot satisfy three commitments `data-processing-register.md` already makes in writing.** (a) The register promises "daily backups + PITR, retained 30 days"; PITR is available only on Pro, Team and Enterprise. (b) It promises a restore rehearsed quarterly, and `scripts/backup-verify.sh` exists to do it — but "database backups are not available for download for Free Plan projects", so the rehearsal cannot be performed at all. (c) Free projects are **paused after a 7-day period of low activity**; a quiet week before launch, or during a slow stretch of the pilot, takes the CRM offline until somebody notices and restores it. None of these is a reason to soften the register — they are the reason the Supabase project must be on Pro before real guest data exists. PITR additionally requires at least a Small compute add-on. Vercel's free tier is for personal, non-commercial use, and the things this deployment depends on — function duration, Fluid Compute, how long runtime logs are kept — are plan-dependent. `[TO BE COMPLETED: the Vercel plan, confirmed against its current terms and its function and log-retention limits]`. An account in an engineer's personal name is a single point of failure with a leaving date. |
| 0.9 | **Region chosen and the reasoning written down** | Engineering decides, the business signs it off | §11.7(1). **Done — Frankfurt.** `berelax-crm-prod` is provisioned in `eu-central-1`, which matches the `regions: ["fra1"]` both Vercel projects already pin, so the database and the API are in one jurisdiction and the privacy notice names one country. The reasoning is recorded in `data-processing-register.md` §4 as this row requires. A first attempt used `ap-south-1` (Mumbai) on latency grounds — the wrong criterion, and it would have split the data store from the API; it was paused empty before any guest data existed. **One action outstanding: delete the paused `berelax-crm` project from the Supabase dashboard.** Pausing is not deletion, and this session's connection has no delete tool. The business still signs the choice off, and `[TO BE COMPLETED: name and date]` in the register's transfer record needs a human name — I provisioned the region, I cannot be the person who accepts it. There is no UAE Supabase region; Frankfurt (`eu-central-1`) has the stronger "adequate protection" argument. **Record the choice and why in `docs/compliance/data-processing-register.md`** — that record is the first thing anyone will ask for. Put the API in the same jurisdiction, so the privacy notice names one country and not two. |
